...
use function OUGCPointsActivityRewards\Core\build_url;
use function OUGCPointsActivityRewards\Core\get_url;
use function OUGCPointsActivityRewards\Core\load_language;
use function OUGCPointsActivityRewards\Core\load_pluginlibrary;
use function OUGCPointsActivityRewards\Core\set_url;
...
set_url('index.php?module=newpoints-activity_rewards');
...
if ($mybb->get_input('action') == 'edit') {
    $sub_tabs['ougc_points_activity_rewards_edit'] = [
        ...
        'link' => build_url(['action' => 'edit', 'pid' => $pid]),
...
if ($mybb->get_input('action') == 'add' || $mybb->get_input('action') == 'edit') {
    ...
    if ($mybb->request_method == 'post') {
        ...
        if ($errors) {
            ...
            admin_redirect(build_url(['action' => 'edit', 'pid' => $pid]));
        }
    } else {
    ...
    $form = new Form(build_url(['action' => $add ? 'add' : 'edit', 'pid' => $pid]), 'post');
    ...
    foreach (['amount', 'hours'] as $key) {
        ...
        $form_container->output_row(
            ...
            $form->generate_numeric_field(
                $key,
                $mybb->get_input($key, MyBB::INPUT_INT)
            )
        );
    }
    ...
    if ($mybb->request_method != 'post') {
        $page->output_confirm_action(
            build_url(['action' => 'delete', 'pid' => $pid, 'confirm' => 1]),
    ...
    $table->construct_header(
        $lang->ougc_points_activity_rewards_admin_status,
    ...
    if (!$db->num_rows($query)) {
        $table->construct_cell(
            $lang->ougc_points_activity_rewards_admin_empty,
            ['colspan' => 3, 'class' => 'align_center']
        );
        ...
        while ($package = $db->fetch_array($query)) {
            ...
            $url = build_url(['action' => 'edit', 'pid' => $package['pid']]);
            ...
            $table->construct_cell(
                '<img src="styles/' . $page->style . '/images/icons/bullet_' . ($package['active'] ? 'on' : 'off') . '.png" /> ',
                ['class' => 'align_center']
            );
            ...
            $popup->add_item($lang->delete, build_url(['action' => 'delete', 'pid' => $package['pid']]));

            $popup->add_item(
                $lang->ougc_points_activity_rewards_admin_toggle,
                build_url(['action' => 'toggle', 'pid' => $package['pid']])
            );
